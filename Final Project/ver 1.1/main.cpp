...
int readPara(char *filename, int *threadNum, int **totalShare, int **periodT, double **alpha, double **pi, double **P0);
...
int readPara(char *filename, int *threadNum, int **totalShare, int **periodT, double **alpha, double **pi, double **P0){
	...
	//get initial price P0
	(*P0) = (double *)calloc( (*threadNum) ,sizeof(double));
	if (!(*P0)){
	  printf("cannot allocate space\n"); retcode = 2; goto BACK;
	}
	fscanf(input,"%s", buffer);
